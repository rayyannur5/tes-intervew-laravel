...
@section('container')
    <div class="flex justify-between">
        <h1 class="text-2xl font-bold">Update Spko</h1>
            ...
            <label class="form-control w-full">
                ...
                <select class="select select-bordered" id="select-employee" name="id_employee">
                    ...
                    @foreach ($employees as $employee)
                        <option value="{{ $employee->id_employee }}"  {{ $employee->id_employee == $spko->employee ? "selected" : "" }}>{{ $employee->nama }}</option>
                    @endforeach
                </select>
                ...
                <input type="date" class="input input-bordered" max="{{ date('Y-m-d') }}" name="trans_date" value="{{ $spko->trans_date }}" id="input-date">
                <div class="label hidden" id="validate-date">
                    <span class="label-text-alt text-red-500">The date field is required</span>
                </div>

                <div class="label mt-2">
                    <span class="label-text">Remarks</span>
                </div>
                <textarea class="textarea textarea-bordered" name="remarks" placeholder="remarks">{{ $spko->remarks }}</textarea>
                ...
                <select class="select select-bordered" id="select-process" name="process">
                    ...
                    @foreach ($process as $item_process)
                        <option value="{{ $item_process }}" {{ $item_process == $spko->process ? "selected" : "" }}>{{ $item_process }}</option>
                    @endforeach
                </select>
    ...
    <script>
        ...
        var product_selected = @json($product_selected);
        updateHtml()
        ...
        $('#create-form').submit(function(event) {
            ...
            if (date == null) {
                validate = false
                $('#validate-date').removeClass('hidden')
            } else {
                $('#validate-date').addClass('hidden')
            }
